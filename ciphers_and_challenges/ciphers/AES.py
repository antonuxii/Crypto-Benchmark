"""
AES implementation taken from https://github.com/boppreh/aes/blob/master/aes.py, full credit goes to the author boppreh
Added features: extended/modified the code to support AES encryption with a custom number of rounds.
"""
...
class AES:
    ...
    def __init__(self, master_key, rounds=None):
        ...
        Args:
            master_key: The encryption key (16, 24, or 32 bytes)
            rounds: Optional custom number of rounds. If None, uses standard AES rounds.
        """
...
def supreme_encryption(key: str, rounds: int, plaintext: str = ""):
    """
    Encrypt with AES for `rounds` rounds.
    ...
    encrypted = cipher.encrypt_block(plain_bytes)
...
def safety_check(rounds: int):
    ...
    cipher = AES(key, rounds=rounds)
    return cipher.encrypt_block(plain).hex()
...
def kpa_target(key: str, rounds: int):
    ...
    ciphertext = cipher.encrypt_block(pt_bytes)
