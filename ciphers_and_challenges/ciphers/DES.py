...
"""
DES implementation taken from https://github.com/Vipul97/des/blob/master/des/des.py, full credit goes to the author Vipul97
Added features: extended/modified the code to support DES encryption with a custom number of rounds.
"""
...
def gen_subkeys(key, rounds=16):
    ...
    for i in range(rounds):
        n_shifts = left_rotate_order[i % len(left_rotate_order)]
...
def des(input_block, subkeys, crypt_type, rounds=16):
    ...
    total_rounds = min(rounds, len(subkeys))
    rounds_iter = range(total_rounds) if crypt_type == 'e' else reversed(range(total_rounds))
    output = initial_permutation

    for i, j in enumerate(rounds_iter, 1):
        print()
        print(f'ROUND {i}:')
...
def crypt(mode, crypt_type, key_file, infile, outfile, iv_file=None, rounds=16):
    bin_in_str = pad(hex_to_bin(infile.read().strip()))
    subkeys = gen_subkeys(hex_to_bin(key_file.read().strip()), rounds)
    bin_out_str = ''

    if mode == 'ecb':
        for i in range(0, len(bin_in_str), BLOCK_SIZE):
            block = bin_in_str[i:i + BLOCK_SIZE]
            bin_out_str += des(block, subkeys, crypt_type, rounds)
    else:
        ...
        for i in range(0, len(bin_in_str), BLOCK_SIZE):
            ...
            output = des(block, subkeys, crypt_type, rounds)
            if crypt_type == 'e':
                last_block = output
            else:
...
def encrypt(plaintext_hex: str, key_hex: str, rounds: int = 16) -> str:
    """Encrypt one 64-bit block given as hex with DES for `rounds` rounds."""
    bin_in = pad(hex_to_bin(plaintext_hex.strip()))[:BLOCK_SIZE]
    subkeys = gen_subkeys(hex_to_bin(key_hex.strip()), rounds)
    bin_out = des(bin_in, subkeys, 'e', rounds)
    return f'{int(bin_out, 2):016X}'.lower()


def decrypt(ciphertext_hex: str, key_hex: str, rounds: int = 16) -> str:
    """Decrypt one 64-bit block given as hex with DES for `rounds` rounds."""
    bin_in = pad(hex_to_bin(ciphertext_hex.strip()))[:BLOCK_SIZE]
    subkeys = gen_subkeys(hex_to_bin(key_hex.strip()), rounds)
    bin_out = des(bin_in, subkeys, 'd', rounds)
    return f'{int(bin_out, 2):016X}'.lower()
...
def supreme_encryption(key: str, rounds: int, plaintext: str = ""):
    """
    Encrypt with DES for `rounds` rounds.
    ...
    with redirect_stdout(io.StringIO()):
        encrypted = encrypt(plaintext, key, rounds)
...
def safety_check(rounds: int):
    ...
    with redirect_stdout(io.StringIO()):
        return encrypt(plain.hex(), key, rounds=rounds)
...
def kpa_target(key: str, rounds: int):
    ...
    where PT is '123456789abcdef0' right-padded to 128-bit.
    """
    pt_bytes =b"123456789abcdef0".ljust(BLOCK_BYTES, b"\x00") # pad "123456789abcdef0"
    with redirect_stdout(io.StringIO()):
        ciphertext = encrypt(pt_bytes, key, rounds)
