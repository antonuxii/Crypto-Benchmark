// SPEEDY implementation taken from https://github.com/Chair-for-Security-Engineering/SPEEDY/tree/main full credit goes to the author Chair-for-Security-Engineering
// only changed Rounds form a global constant to a parameter for each function
...
void Encrypt(const unsigned char *Plaintext, const unsigned char *Key, unsigned char *Ciphertext, int rounds)
{
    // Key Schedule
    unsigned char RoundKey[24] = {0};
    unsigned char RoundKeysXORConstants[MAX_ROUNDS+1][24] = {{0}};
    for(int i=0; i<24; i++)
    {
        RoundKey[i] = Key[i];
...
void Decrypt(const unsigned char *Ciphertext, const unsigned char *Key, unsigned char *Plaintext, int rounds)
{
    // Key Schedule
    unsigned char RoundKey[24] = {0};
    unsigned char RoundKeysXORConstants[MAX_ROUNDS+1][24] = {{0}};
...
int main()
{
    ...
    int rounds = 7; /* Put a number between 1 and 9 here */

    print_state(Plaintext);
    Encrypt(Plaintext, Key, Ciphertext, rounds);
    print_state(Ciphertext);
    Decrypt(Ciphertext, Key, Plaintext, rounds);
    print_state(Plaintext);
